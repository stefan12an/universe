...
class AuthViewModel @Inject constructor(private val firebaseRepository: FirebaseRepository) :
    ...
    init {
        ...
        viewModelScope.launch {
            val universities = RemoteConfigHelper.getUniversitiesList()
            _uiState.value = uiState.value!!.copy(universities = universities)
        }
    }
    ...
    private fun register(email: String?, password: String?) {
        ...
        if(!validateEmail(email)){
            pushSideEffect(AuthSideEffect.Feedback("Please use a valid university email"))
            _uiState.value = uiState.value!!.copy(loading = false)
            return
        }
