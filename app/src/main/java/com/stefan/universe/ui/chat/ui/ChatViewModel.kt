...
class ChatViewModel @Inject constructor(
    ...
    private fun initiateProfileView() {
        viewModelScope.launch {
            val user = firebaseRepository.getUser()
            if (user.succeeded) {
                pushSideEffect(ChatSideEffects.GoToProfile(chatWrapper.chat.participants.first { it != user.data?.uid }))
            } else {
                pushSideEffect(ChatSideEffects.Feedback("Failed to view profile"))
            }
        }
    }
...
sealed class ChatUserIntent : UserIntent {
    ...
    data object ViewProfile : ChatUserIntent()
}
...
sealed class ChatSideEffects : SideEffect {
    ...
    data class GoToProfile(val userId: String?) : ChatSideEffects()
}