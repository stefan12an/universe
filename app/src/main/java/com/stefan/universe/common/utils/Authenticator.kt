...
interface Authenticator {
    ...
    suspend fun editProfile(displayName: String?, uri: Uri?): Resource<Boolean>
...
class FirebaseAuthenticator @Inject constructor() : Authenticator {
    ...
    override suspend fun editProfile(displayName: String?, uri: Uri?) = runCatching {
    ...
    override suspend fun updateAdditionalUserDetailsInDb(user: User) = runCatching {
        if (user.uid.isNotEmpty()) {
            db.collection("Users").document(user.uid)
                .update(mapOf("displayName" to user.displayName, "photoUri" to user.photoUri))
                .await()
            Resource.success(true)
        } else {
